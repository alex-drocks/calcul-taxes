        ...
        <table>
          <thead className="results-table--header">
            <tr>
              ...
              ></th>
              <th id="sumTps" className="tps col-sum" title="Montant de la taxe Fédérale"></th>
              <th id="sumTvq" className="tvq col-sum" title="Montant de la taxe provinciale"></th>
              <th
                id="sumTotal"
                className="total col-sum"
                title="Somme du MONTANT + taxe fédérale + taxe provinciale = TOTAL"
              ></th>
              <th className="province col-sum" title="Province sélectionnée pour le calcul">
                Résultats de vos calculs de taxes
              </th>
              <th
            ...
            <tr>
              ...
              <th className="province" title="Province sélectionnée pour le calcul">
                PROVINCE ET TAUX
              </th>
              <th
        ...
        <div className="export-btns">
          ...
          >
            ...
            <svg xmlns="http://www.w3.org/2000/svg" viewBox="0 0 24 24">
              <path d="M24 5h-4V0H4v5H0v13h4v6h9.519c2.947 0 6.029-3.577 6.434-6H24V5zM6 2h12v3H6V2zm8.691 16.648S16.16 22 12.691 22H6v-8h12v2.648c0 3.594-3.309 2-3.309 2zM21.5 8c-.276 0-.5-.224-.5-.5s.224-.5.5-.5.5.224.5.5-.224.5-.5.5zM16 17H8v-1h8v1zm-3 1H8v1h5v-1z" />
            </svg>
          </button>
          <button
            id="excelBtn"
...
function deleteTableRow(rowToDelete) {
  if (rowToDelete) {
    const subtractValues = {
      montant: -roundNumber(rowToDelete.querySelector("td.montant").textContent),
      tps: -roundNumber(rowToDelete.querySelector("td.tps").textContent),
      tvq: -roundNumber(rowToDelete.querySelector("td.tvq").textContent),
      total: -roundNumber(rowToDelete.querySelector("td.total").textContent)
    };
    rowToDelete.remove();
    updateRowCount();
    updateColSums(subtractValues);
  }
}
...
function updateColSums(newRoundedRowValues) {
  ...
  sumMontantEl.textContent = roundNumber(
    (Number(sumMontantEl.textContent) || 0) + newRoundedRowValues.montant
  );
  ...
  sumTotalEl.textContent = roundNumber(
    (Number(sumTotalEl.textContent) || 0) + newRoundedRowValues.total
  );
}
